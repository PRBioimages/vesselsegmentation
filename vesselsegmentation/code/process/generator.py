...
def get_training_and_validation_generators(data_file, batch_size, n_labels, training_keys_file, validation_keys_file,
                                           data_split, overwrite=False, labels=None, augment=False,
    ...
    :param skip_blank: If True, any blank (all-zero) label images/patches will be skipped by the data generator.  #可以考虑使用
    ...
    training_list, validation_list = get_split(data_file, data_split,
                                               overwrite=overwrite,
...
def get_split(data_file, data_split, training_file, validation_file, overwrite=False):
    """
...
def split_list(input_list, split=4, shuffle_list=False):
    if shuffle_list:
        shuffle(input_list)
    training = input_list[:(int(split) - 1)]
    testing = input_list[(int(split) - 1):]
...
def data_generator(data_file, index_list, batch_size=1, n_labels=1, labels=None, augment=False, augment_flip=True,
    ...
    while True:
        ...
        while len(index_list) > 0:
            ...
            # i=i+1
            if len(x_list) == batch_size or (
                    len(index_list) == 0 and len(x_list) > 0):
...
def add_data(x_list, y_list, data_file, index, augment=False, augment_flip=False, augment_distortion_factor=0.25,
    ...
    if not skip_blank or np.any(truth == 1):
        if augment:
            if patch_shape is not None:
                affine = data_file.root.affine[index[0]]
            else:
                affine = data_file.root.affine[index]
            data, truth = augment_data(data, truth, affine, patch_shape=patch_shape)
        if permute:
