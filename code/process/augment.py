...
def augment_data(data, truth,affine,patch_shape=None):

    data_list = list()
    aug = get_augmentation(patch_shape)
    image = get_image(data[0], affine)
    truth_image = get_image(truth, affine)
    new_data = np.copy(image.get_data())
    new_truth = np.copy(truth_image.get_data())
    data = {'image':new_data, 'mask': new_truth}
    aug_data = aug(**data)
    img, lbl = aug_data['image'], aug_data['mask']
    new_img_like(image, data=img)
    new_img_like(truth_image, data=lbl)
    data=image.get_data()
    truth_data=truth_image.get_data()
...
def generate_permutation_keys():
    ...
    return set(itertools.product(itertools.combinations_with_replacement(range(2), 2), range(2), range(2), range(2), range(1)))
