...
from dvn.metrics import dice, dice_score,metric_dice,accuracy_bin,sensitivity,specificity,precision
...
def load_old_model(model_file):
    ...
    custom_objects = {'soft_dice': soft_dice, 'metric': dice()}
    try:
...
def run_validation_case(data_index, output_dir, model, data_file, training_modalities,
    ...
    if origin_shape is not None:
        prediction1 = np.zeros((prediction.shape[0],prediction.shape[1],origin_shape[2], origin_shape[1],origin_shape[0]), dtype=prediction.dtype)
        mask1=np.zeros((origin_shape[2], origin_shape[1],origin_shape[0]), dtype=np.uint8)
        prediction1[:,:,:origin_shape[2], :origin_shape[1], :origin_shape[0]]= prediction[:,:,:origin_shape[2], :origin_shape[1], :origin_shape[0]]
        if data_file.root.truth.shape[0] != 0:
            mask = data_file.root.truth[data_index][0]
            mask1[:origin_shape[2], :origin_shape[1], :origin_shape[0]] = mask[:origin_shape[2],:origin_shape[1], :origin_shape[0]]
    prediction_image,data_array = prediction_to_image(prediction1, affine, label_map=output_label_map, threshold=threshold,labels=labels)
    if data_file.root.truth.shape[0] != 0:
        label_index = []
        for ind in range(mask1.shape[2]):
            if np.any(mask1[:, :, ind] != 2) == True:
                label_index.append(ind)
        print(label_index)
        data1 = data_array[:, :, label_index]
        mask1 = mask1[:, :, label_index]
    ...
    if isinstance(prediction_image, list):
        ...
        prediction_image.to_filename(os.path.join(output_dir,name+"{}.nii.gz".format(data_index)))
...
def run_validation_cases(training_keys_file, model_file, training_modalities, labels, hdf5_file,
    ...
    for k, p in enumerate([dice1, acc_voxel1, sensitivity2, specificity2, precision2]):
        ...
        Sum[k] = sum(p.values())
        Avg[k] = Sum[k] / Len[k]
    path = output_dir+ '/metric_other.txt'
    f = open(path, 'w')
